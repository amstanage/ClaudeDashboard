...
final class DatabaseServiceTests: XCTestCase {
    ...
    func testInsertAndFetchSession() throws {
        let session = SessionRecord(id: "test-1", projectPath: "/Users/test/project", startedAt: Date(timeIntervalSince1970: 1000), endedAt: Date(timeIntervalSince1970: 2000), model: "claude-opus-4-6", totalInputTokens: 100, totalOutputTokens: 50, firstMessage: "Hello")
    ...
    func testUpsertDailyStats() throws {
        ...
        try db.upsertDailyStats(date: date, inputTokens: 100, outputTokens: 50, sessions: 1)
        try db.upsertDailyStats(date: date, inputTokens: 200, outputTokens: 80, sessions: 1)
    ...
    func testFetchDailyStatsRange() throws {
        ...
        try db.upsertDailyStats(date: day1, inputTokens: 10, outputTokens: 5, sessions: 1)
        try db.upsertDailyStats(date: day2, inputTokens: 20, outputTokens: 10, sessions: 2)
        try db.upsertDailyStats(date: day3, inputTokens: 30, outputTokens: 15, sessions: 1)
        let stats = try db.fetchDailyStats(from: day1, to: day2)
        XCTAssertEqual(stats.count, 2)
    }

    func testSearchSessions() throws {
        let s1 = SessionRecord(id: "s1", projectPath: "/p", startedAt: Date(), endedAt: nil, model: "claude-opus-4-6", totalInputTokens: 10, totalOutputTokens: 5, firstMessage: "Help me with Swift")
        let s2 = SessionRecord(id: "s2", projectPath: "/p", startedAt: Date(), endedAt: nil, model: "claude-sonnet-4-6", totalInputTokens: 10, totalOutputTokens: 5, firstMessage: "Write Python code")
